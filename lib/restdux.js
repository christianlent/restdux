...
function CombineCalls(resource, callBucket) {
    var calls = Object.keys(callBucket).map(function (name) { return callBucket[name].setName(name); });
    var actions = {};
    ...
    var cacheread = function cachereadF(cacheOptions, id, sent, urlParameters) {
        ...
        return function crReturn(dispatch, getState) {
            function batchProcess() {
                ...
                var latest = 0;
                var batchedMetaBag = {};
                filterMax(Object.keys(batchState.meta), batchSizeMax, function (targetId) { return isQueued(batchState.meta[targetId]); })
            ...
            var entity = getEntity(state, id);
            if (entity && meta && isValid(__assign(__assign({}, options.cacheOptions), cacheOptions), state, id)) {
