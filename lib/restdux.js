...
function CombineResource(resource, callBucket) {
    ...
    return {
        actions: __assign(__assign({}, resource.actions), calls.actions),
        reducer: combineReducers([resource.reducer, calls.reducer]),
        types: __assign(__assign({}, resource.types), calls.types),
    };
}
...
function CombineCalls(resource, callBucket) {
    var calls = Object.keys(callBucket).map(function (name) {
        return callBucket[name].setName(name);
    });
    var actions = {};
...
function idUrlBuilder(rootUrl) {
    return function urlF(id, urlParameters) {
        ...
        return rootUrl + "/" + id + getQueryString(urlParameters);
    };
}
exports.idUrlBuilder = idUrlBuilder;
function urlBuilder(rootUrl) {
    return function urlF(id, urlParameters) {
        rootUrl = typeof rootUrl === "function" ? rootUrl() : rootUrl;
        rootUrl = rootUrl.replace(/\/$/, "");
        return rootUrl + getQueryString(urlParameters);
    };
}
function Call(newOptions) {
    ...
    var options = __assign(__assign({}, defaultCallOptions), newOptions);
    ...
    return {
        ...
        name: name,
        reducer: reducer,
        setName: function (newName) { return Call(__assign(__assign({}, newOptions), { name: newName })); },
        types: types,
    };
}
exports.Call = Call;
...
function Resource(options) {
    options = __assign(__assign({}, defaultResourceOptions), options);
    var idField = options.idField;
    var resourceName = options.name;
    var transformIndex = options.transformIndex || (function (result) { return result; });
    var calls = {
        create: Call(__assign(__assign({}, options), { method: options.methodCreate, name: "create", resourceName: resourceName, url: urlBuilder(options.rootUrl) })),
        delete: Call(__assign(__assign({}, options), { method: options.methodDelete, name: "delete", resourceName: resourceName, url: idUrlBuilder(options.rootUrl) })),
        index: Call(__assign(__assign({}, options), { method: options.methodIndex, name: "index", resourceName: resourceName, url: urlBuilder(options.rootUrl) })),
        read: Call(__assign(__assign({}, options), { method: options.methodRead, name: "read", resourceName: resourceName, url: idUrlBuilder(options.rootUrl) })),
        update: Call(__assign(__assign({}, options), { method: options.methodUpdate, name: "update", resourceName: resourceName, url: idUrlBuilder(options.rootUrl) })),
    };
    var types = {
    ...
    var cacheread = function cachereadF(cacheOptions, id, sent, urlParameters) {
        ...
        return function crReturn(dispatch, getState) {
            function batchProcess() {
                ...
                var batchSizeMax = options.batchSizeMax || defaultResourceOptions.batchSizeMax;
                var latest = 0;
                var batchedMetaBag = {};
                filterMax(Object.keys(batchState.meta), batchSizeMax, function (targetId) {
                    return isQueued(batchState.meta[targetId]);
                ...
                if (!Object.keys(batchedMetaBag).length ||
                ...
                dispatch(actions.index(undefined, undefined, parameters))
                    ...
                    Object.keys(batchedMetaBag).forEach(function (targetId) {
                        ...
                        placeholderEntity.handler.resolve(__assign(__assign({}, action), { result: target }));
                    });
                    queueUpdate(batchProcess);
                })
                    ...
                    Object.keys(batchedMetaBag).forEach(function (targetId) {
                        var placeholderEntity = batchedMetaBag[targetId];
                        dispatch(actions.readFailure(err.response, {}, targetId));
                        if (!placeholderEntity.handler ||
            ...
            if (entity &&
                meta &&
                isValid(__assign(__assign({}, options.cacheOptions), cacheOptions), state, id)) {
    ...
    function updateEntity(state, id, newData, newMeta) {
        ...
        if (newData) {
            var entity = getEntity(state, id);
            state.data[id] = __assign(__assign({}, entity), newData);
        }
        if (newMeta) {
            var meta = getMeta(state, id);
            state.meta[id] = __assign(__assign({}, meta), newMeta);
        }
        return state;
    }
    ...
    function reducer(state, action) {
        if (state === void 0) { state = exports.DefaultStateBucket; }
        var id;
        switch (action.type) {
